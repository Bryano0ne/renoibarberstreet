...
const PHOTOS_REELLES = [
  {
    src: "/images/galerie/tapers-fades.jpg",
    titre: "Tapers & Fades",
    styles: ["Low Taper", "Mid Taper", "High Taper", "High Fade", "Mid Fade", "Low Fade"],
    categorie: "Dégradé",
    wide: true,
  },
  {
    src: "/images/galerie/twist-locks.jpg",
    titre: "Twist & Locks",
    styles: ["Twist", "Locks"],
    categorie: "Afro",
    wide: false,
...
// ─── Placeholders pour les autres coupes ───────────────────────
const PLACEHOLDER_ITEMS = [
  { label: "Dégradé Classique",  salon: "ZAD",   icon: "💈", cat: "Dégradé" },
  { label: "Afro Sculpté",       salon: "SAABA",  icon: "✂️", cat: "Afro" },
  { label: "Barbe Sculptée",     salon: "ZAD",   icon: "🪒", cat: "Barbe" },
  { label: "Combo Coupe+Barbe",  salon: "SAABA",  icon: "💈", cat: "Combo" },
  { label: "Rasage Traditionnel",salon: "ZAD",   icon: "🪒", cat: "Barbe" },
  { label: "Coupe Enfant",       salon: "SAABA",  icon: "✂️", cat: "Afro" },
  { label: "Dégradé Américain",  salon: "ZAD",   icon: "💈", cat: "Dégradé" },
  { label: "Soin Barbe",         salon: "SAABA",  icon: "🌿", cat: "Soins" },
  { label: "Ligne au Rasoir",    salon: "ZAD",   icon: "🪒", cat: "Barbe" },
  { label: "Afro Naturel",       salon: "SAABA",  icon: "✂️", cat: "Afro" },
];
  ...
  return (
    <main className="pt-24 pb-20 px-6 min-h-screen">
      <div className="max-w-6xl mx-auto">
        ...
        {/* ── Photos réelles en vedette ── */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
          {PHOTOS_REELLES.map((photo) => (
            <div
              key={photo.src}
              className="group relative rounded-2xl overflow-hidden border border-[#2A2A2A] hover:border-[#C9A84C]/50 transition-all duration-300 cursor-pointer"
            ...
            >
              ...
              <div className="absolute bottom-0 left-0 right-0 p-5">
                <p className="text-[10px] text-[#C9A84C] tracking-[0.3em] uppercase mb-1">{photo.categorie}</p>
                <h3 className="text-xl font-black text-[#F5F0E8] mb-3">{photo.titre}</h3>
                {/* Tags des noms de coupes */}
                <div className="flex flex-wrap gap-1.5">
                  {photo.styles.map((style) => (
                    <span
                      key={style}
                      className="text-[10px] font-semibold px-2.5 py-1 rounded-full bg-[#C9A84C]/15 border border-[#C9A84C]/30 text-[#C9A84C]"
                    >
                      {style}
                    </span>
                  ))}
        ...
        {/* ── Grille placeholders autres coupes ── */}
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 mb-12">
          {PLACEHOLDER_ITEMS.map((item) => (
            ...
            >
              <span className="text-4xl opacity-20 group-hover:opacity-50 group-hover:scale-125 transition-all duration-500">
                {item.icon}
              </span>
              <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-[#0A0A0A]/90 to-transparent p-3 translate-y-full group-hover:translate-y-0 transition-transform duration-300">
                <p className="text-[11px] font-semibold text-[#F5F0E8]">{item.label}</p>
                <p className="text-[10px] text-[#C9A84C]">{item.salon}</p>
              </div>
            </div>
          ))}
        </div>
